...
d_ids = {}
def get_performance_test(topk_val, ep_val1, ep_val2, second_pass=False, llm_label='', prev_prob = 0, prev_lab = '', counter=0):
    #print(test_dataset)
    torch.use_deterministic_algorithms(True)
    set_seed(42)
    ...
    for ii,ti, gol, ids in tqdm(test_dataset_,disable=second_pass):
        ...
        print(grad_.shape, ti.shape)
       
        TopK = []
        epsilon = []
        for i in op.topk(topk_val).indices:
            

            #if val>0:
           
            TopK.append(tokenizer1.decode(i).strip().lower())
        ...
        for i in op.topk(50257).indices:
            ...
            print(torch.nn.functional.cosine_similarity(ti,k.T))
            ...
            if val>=ep_val1 and val<=ep_val2:
                epsilon.append(tokenizer1.decode(i).strip().lower())

        TopK = set(TopK)
        epsilon = set(epsilon)
        
        num = len(TopK.intersection(epsilon))
        denom = len(TopK.union(epsilon))
...
for i in pairs:
    ...
    set_seed(42)
    a,b,c = 3500 , i[0], i[1]
    jaccard = get_performance_test(a,b,c)

   
    print(a,b,c)
    print('Jaccard Similarity mean is {}'.format(np.mean(jaccard)))
    print('Jaccard Similarity std is {}'.format(np.std(jaccard)))