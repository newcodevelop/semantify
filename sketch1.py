...
def get_performance_test(second_pass=False, llm_label='', prev_prob = 0, prev_lab = '', counter=0):
    ...
    for ii,ti, gol, ids in tqdm(test_dataset_,disable=second_pass):
        ...
        for i in range(1):
            ...
            output_aware_tokens = full[i*3500:(i+1)*3500]
...
print(f1_score(gl_test, all_labs, average='macro'), f1_score(gl_test, all_labs, average='macro'))
print(accuracy_score(gl_test, all_labs), accuracy_score(gl_test, all_labs))


with open('iclr_3500_0.05_ball_on_on_I.pkl', 'wb') as f:
